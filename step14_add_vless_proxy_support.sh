...
# Update menu/help text to show VLESS support.
text = text.replace(
'''Currently fully supported for proxy test/start:"
  echo "- ss:// Shadowsocks"''',
'''Currently fully supported for proxy test/start:"
  echo "- ss:// Shadowsocks"
  echo "- vless:// VLESS"'''
...
echo "Now run:"
echo "git add . && git commit -m 'Add VLESS support to proxy mode' && git push"
