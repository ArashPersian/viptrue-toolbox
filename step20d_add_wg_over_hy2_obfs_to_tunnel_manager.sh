...
# Step 20-D
# Add WireGuard over Hysteria2 obfs/salamander tools to VIPTrue Tunnel Manager.
# Run from viptrue-toolbox root.

if [[ ! -f "lib/ui.sh" || ! -d "modules" ]]; then
  echo "ERROR: Run this script from viptrue-toolbox root directory."
  echo "Example: cd ~/viptrue-toolbox && bash /root/step20d_add_wg_over_hy2_obfs_to_tunnel_manager.sh"
  exit 1
fi

BASE_DIR="$(pwd)"
TS="$(date +%F-%H%M%S)"
BACKUP_DIR="/root/viptrue-toolbox-patch-backup-step20d-${TS}"
mkdir -p "$BACKUP_DIR"
cp -a "$BASE_DIR" "$BACKUP_DIR/viptrue-toolbox" 2>/dev/null || true

echo "0.1.6" > VERSION
mkdir -p modules/utility

cat > modules/utility/tunnel-wg-hy2.sh <<'HY2MODULE'
#!/usr/bin/env bash
set -Eeuo pipefail
...
chmod +x modules/utility/tunnel-wg-hy2.sh
...
candidates = []
for p in [Path('modules/utility/04-tunnel-manager.sh'), Path('modules/utility/04-tunnel-tools.sh')]:
    if p.exists(): candidates.append(p)
candidates += sorted(Path('modules/utility').glob('*tunnel*.sh'))
seen=set(); ordered=[]
for p in candidates:
    if p not in seen:
...
for p in ordered:
    ...
    if 'Tunnel' in txt and ('Back' in txt or 'Exit' in txt) and 'case' in txt:
        target=p; break

if target is None:
    target=Path('modules/utility/04-tunnel-manager.sh')
    target.write_text('''#!/usr/bin/env bash\nset -Eeuo pipefail\nBASE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"\nsource "$BASE_DIR/lib/ui.sh"\nwhile true; do\n  title\n  echo -e "${CYAN}Tunnel Manager${NC}"\n  line\n  echo "1. WireGuard over Hysteria2 obfs"\n  echo "0. Back"\n  read -r -p "Enter your choice [0-1]: " choice\n  case "$choice" in\n    1) bash "$BASE_DIR/modules/utility/tunnel-wg-hy2.sh" ;;\n    0) break ;;\n    *) echo -e "${RED}Invalid choice.${NC}"; sleep 1 ;;\n  esac\ndone\n''')

text=target.read_text()
if 'WireGuard over Hysteria2 obfs' not in text:
    text=re.sub(r'(\n\s*echo\s+["\']0\.\s*(?:Back|Exit)["\'])', r'\n  echo "8. WireGuard over Hysteria2 obfs"\1', text, count=1)
    text=re.sub(r'Enter your choice \[0-\d+\]', 'Enter your choice [0-8]', text, count=1)
    handler='''\n    8)\n      bash "$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/modules/utility/tunnel-wg-hy2.sh"\n      ;;'''
    text=re.sub(r'(\n\s*0\)\s*\n\s*(?:break|return)\s*\n\s*;;)', handler + r'\1', text, count=1)
    if 'tunnel-wg-hy2.sh' not in text:
        raise SystemExit(f'Could not patch case handler in {target}.')
    target.write_text(text)
print(f'Patched Tunnel Manager module: {target}')
PY

python3 - <<'PY'
from pathlib import Path
import re
utility=Path('menus/utility.sh')
if not utility.exists():
    print('menus/utility.sh not found; skipping utility menu patch.')
    raise SystemExit
text=utility.read_text()
if 'Tunnel Manager' in text or 'Tunnel Tools' in text:
    print('Utility menu already has a tunnel entry.')
    raise SystemExit
if 'echo "3. Offline Assets / Local Installer"' in text:
    text=text.replace('echo "3. Offline Assets / Local Installer"', 'echo "3. Offline Assets / Local Installer"\n  echo "4. Tunnel Manager"')
else:
    text=re.sub(r'(echo "2\..*?")', r'\1\n  echo "4. Tunnel Manager"', text, count=1)
text=re.sub(r'Enter your choice \[0-\d+\]', 'Enter your choice [0-4]', text, count=1)
if '04-tunnel-manager.sh' not in text:
    text=re.sub(r'(\n\s*0\)\s*\n\s*break\s*\n\s*;;)', r'\n    4)\n      bash "$BASE_DIR/modules/utility/04-tunnel-manager.sh"\n      ;;\1', text, count=1)
utility.write_text(text)
print('Patched menus/utility.sh')
PY

find . -type f -name "*.sh" -print0 | while IFS= read -r -d '' f; do bash -n "$f"; done
chmod +x modules/utility/*.sh 2>/dev/null || true

echo
echo "✅ Step 20-D completed successfully."
echo "Version is now: $(cat VERSION)"
echo "Backup: $BACKUP_DIR"
echo
echo "Next commands:"
echo "git add ."
echo "git commit -m 'Add WireGuard over Hysteria2 obfs tunnel manager tools'"
echo "git push"
echo
echo "Then test from GitHub:"
