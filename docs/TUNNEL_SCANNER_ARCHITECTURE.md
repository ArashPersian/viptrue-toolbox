...
2. The Foreign side prints a `VIPTRUE_SCAN_BUNDLE`.
3. Run the scanner on the Iran / Entry server.
4. Paste the scan bundle, or enter the same values manually.
5. Review ranked engines and build only the selected implemented engine.
...
listening, the scanner warns but does not fail unless a future explicit live
test requires a listener.

## Scoring Model

The scanner uses a standardized score model and prints a ranked table:
