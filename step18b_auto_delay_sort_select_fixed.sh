...
pattern_select = r'select_active_outbound\(\)\s*\{.*?\n\}\n(?=\n[a-zA-Z0-9_]+\(\)\s*\{)'
text, count_select = re.subn(pattern_select, new_select + "\n", text, count=1, flags=re.S)

if count_select == 0:
    raise SystemExit("Could not replace select_active_outbound function")
...
pattern_menu = r'config_links_menu\(\)\s*\{.*?\n\}\n(?=\n[a-zA-Z0-9_]+\(\)\s*\{)'
text, count_menu = re.subn(pattern_menu, new_menu + "\n", text, count=1, flags=re.S)

if count_menu == 0:
    raise SystemExit("Could not replace config_links_menu function")
...
echo
echo "✅ Step 18-B completed successfully."
echo "Now run:"
