...
get_node_port() {
  ...
  for key in PORT NODE_PORT API_PORT PG_NODE_PORT LISTEN_PORT TLS_PORT; do
...
show_connection_info() {
  ...
  local svc public_ip node_port api_key
  ...
  echo "https://localhost:${node_port}"
  echo
...
echo "Now run:"
echo "git add . && git commit -m 'Add PasarGuard node connection info' && git push"
