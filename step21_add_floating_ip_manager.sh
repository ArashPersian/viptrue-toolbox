...
MODULE_DIR="modules/utility"
MODULE_FILE="$MODULE_DIR/06-floating-ip-manager.sh"

if [[ ! -d "$MODULE_DIR" ]]; then
  echo "ERROR: Run this from viptrue-toolbox root. Missing: $MODULE_DIR"
  exit 1
fi

mkdir -p "$MODULE_DIR"

cat > "$MODULE_FILE" <<'EOF_MODULE'
#!/usr/bin/env bash
set -Eeuo pipefail
...
Step 21 applied.
Direct test:
  bash modules/utility/06-floating-ip-manager.sh

If your Utility Tools menu auto-discovers modules, Floating IP Manager should appear automatically.
If it does not appear, send the Utility Tools menu file/output and we patch the menu entry exactly.
MSG
