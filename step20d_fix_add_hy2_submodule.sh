...
MAIN="modules/utility/04-tunnel-manager.sh"
SUB="modules/utility/tunnel-wg-hy2-obfs.sh"

if [[ ! -f "$MAIN" ]]; then
  echo "ERROR: Run from viptrue-toolbox root. Missing $MAIN"
  exit 1
fi

BACKUP="$MAIN.bak.step20d-fix.$(date +%Y%m%d-%H%M%S)"
cp "$MAIN" "$BACKUP"
echo "Backup: $BACKUP"
...
SUBEOF
chmod +x "$SUB"
echo "Created submodule: $SUB"

python3 - <<'PY'
from pathlib import Path
p = Path('modules/utility/04-tunnel-manager.sh')
s = p.read_text()
if 'WireGuard over Hysteria2 obfs/salamander' not in s:
    s = s.replace('  echo "6. Live tcpdump"\n  echo "0. Back"',
                  '  echo "6. Live tcpdump"\n  echo "7. WireGuard over Hysteria2 obfs/salamander"\n  echo "0. Back"')
    s = s.replace('read -r -p "Enter your choice [0-6]: " choice',
                  'read -r -p "Enter your choice [0-7]: " choice')
    s = s.replace('    6) live_tcpdump ;;\n    0) break ;;',
                  '    6) live_tcpdump ;;\n    7) bash "$BASE_DIR/modules/utility/tunnel-wg-hy2-obfs.sh" ;;\n    0) break ;;')
else:
    if 'bash "$BASE_DIR/modules/utility/tunnel-wg-hy2-obfs.sh"' not in s:
        s = s.replace('    6) live_tcpdump ;;\n    0) break ;;',
                      '    6) live_tcpdump ;;\n    7) bash "$BASE_DIR/modules/utility/tunnel-wg-hy2-obfs.sh" ;;\n    0) break ;;')
p.write_text(s)
PY
...
echo
echo "Step 20-D fix applied successfully."
echo "Test: bash modules/utility/04-tunnel-manager.sh"
